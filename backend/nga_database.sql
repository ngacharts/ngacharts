...
SELECT 
  ...
  k.GD AS GD, k.PR AS PR, IF(k.PP IS NULL, NULL, FLOOR(ABS(k.PP))) as PPdeg, IF(k.PP IS NULL, NULL, ROUND((ABS(k.PP) - FLOOR(ABS(k.PP))) * 60)) as PPmin, IF(k.PP IS NULL, NULL, IF(k.PP >= 0, 1, -1)) AS PPhemi, k.UN AS UN, k.SD AS SD, k.DTMx AS DTMx, k.DTMy AS DTMy, k.DTMdat AS DTMdat, k.changed AS changed, k.changed_by AS changed_by, k.bsb_type AS bsb_type,
